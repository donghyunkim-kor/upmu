...
with tab2:
  ...
  if selected_eng_file and selected_eng_file in saved_eng_files:
    ...
    if os.path.exists(file_path):
      ...
      st.subheader(
          f"2. 조건 설정 (대상 파일: {selected_eng_file} / 총"
          f" {len(df_engineer)}건 레코드)"
      )
      ...
      if not filtered_df.empty and "이름" in filtered_df.columns:
        ...
        st.subheader(
            f"🎯 조건 충족 적합 인원: 총 {len(summary_df)}명 (상세 이력"
            f" {len(filtered_df)}건)"
        )
...
with tab3:
  ...
  if selected_perf_file and selected_perf_file in saved_perf_files:
    ...
    if os.path.exists(file_path):
      ...
      st.subheader(
          f"2. 실적 필터링 (선택된 파일: {selected_perf_file} / 총"
          f" {len(df_perf)}건)"
      )
...
with tab4:
  ...
  with col_up_c:
    uploaded_contract = st.file_uploader(
        "계약관리 엑셀 파일(.xlsx) 업로드", type=["xlsx"], key="contract_uploader"
    )
  ...
  with col_sel_c:
    if saved_contract_files:
      selected_contract_file = st.selectbox(
          "📁 계약 파일 목록 선택", saved_contract_files, key="contract_select"
      )
      if st.button("🗑️ 선택한 계약 파일 삭제", key="del_contract"):
        ...
        if delete_saved_file(file_path_to_del):
          ...
          for key in ["contract_select", "contract_uploader"]:
            if key in st.session_state:
              del st.session_state[key]
          st.success(f"🗑️ '{selected_contract_file}' 파일이 삭제되었습니다.")
          st.rerun()
    else:
  ...
  if selected_contract_file and selected_contract_file in saved_contract_files:
    ...
    if os.path.exists(file_path):
      try:
        ...
        st.error(
            "엑셀 파일에서 '계약입력' 시트를 읽어오지 못했습니다. 시트명을"
            f" 확인해주세요. (에러: {e})"
        )
        df_contract = None
      ...
      if df_contract is not None:
        ...
        if "계약명" in df_contract.columns:
          ...
          selected_contract = st.selectbox(
              "🎯 조회할 계약(공사)을 선택하세요:",
              contract_list,
              key="selected_contract_box",
          )

          if selected_contract:
            sub_df = df_contract[
                df_contract["계약명"] == selected_contract
            ].copy()

            first_row = sub_df.iloc[0]
            client = (
                first_row["발주처"]
                if "발주처" in df_contract.columns
                and pd.notna(first_row["발주처"])
                else "-"
            )

            share_ratio = 1.0
            if "지분율" in df_contract.columns and pd.notna(
                first_row["지분율"]
            ):
              try:
                # 퍼센트 기호나 소수점 형태 대응
            ...
            if "낙찰(계약)금액" in sub_df.columns:
              total_contract_amount = pd.to_numeric(
                  sub_df["낙찰(계약)금액"], errors="coerce"
              ).sum()
            ...
            if my_col and my_col in sub_df.columns:
              my_contract_amount = pd.to_numeric(
                  sub_df[my_col], errors="coerce"
              ).sum()
            ...
            with col1:
              st.metric(
                  "총 공사 계약금액", f"{total_contract_amount:,.0f} 원"
              )
            with col2:
              st.metric("당사 지분율", f"{share_ratio * 100:.1f}%")
            with col3:
              st.metric("당사 총 계약금액", f"{my_contract_amount:,.0f} 원")
            with col4:
          ...
          st.error(
              "선택한 엑셀 파일의 '계약입력' 시트에 '계약명' 컬럼이 존재하지"
              " 않습니다."
          )
